...
3. **0.3 Linux Native**：0.3a 实现 V4L2 CPU VideoFrame 采集；**0.3b（已完成开发验收，稳定包仍为 0.2.0）**实现 ALSA CPU AudioFrame 采集；0.3c 使用开发机可选 FFmpeg Backend 验证 libx264 H.264、libx265 H.265、AAC 和 libopus Opus 编码，不作为 ARM 产品依赖；0.3d 同时采集音视频，在共同单调时钟域内测量偏差和漂移，首期不自动校正。
