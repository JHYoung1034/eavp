...
完成标准包括 Debug/Release CTest、ASan/UBSan、安装消费工程和 aarch64 交叉编译。安装消费工程必须通过 `find_package(EAVP 0.2 CONFIG REQUIRED)` 独立配置、编译和运行，覆盖 CPU Buffer、VideoFormat、BackendRegistry 与 Reference Provider 选择。交叉编译只验证生成与链接，不执行目标程序；工具链不可用时必须保留未通过记录和原始错误，不能将其写为通过。
