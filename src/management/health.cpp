...
Status HealthManager::report(const std::string& component, HealthStatus status,
    ...
    std::map<std::string, HealthComponent>::iterator existing = components_.find(component);
    if (existing == components_.end()) {
        components_.insert(std::make_pair(component, HealthComponent(status, message)));
    } else {
